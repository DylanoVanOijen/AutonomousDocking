...
class RewardComputer():
    def __init__(self, approach_direction, reward_type, reward_parameters, docking_ports, docking_settings, controller_type):
        ...
        if self.controller_type == "translational":
            self.trans_or_rot_reward = self.get_trans_reward
        elif self.controller_type == "rotational":
            self.trans_or_rot_reward = self.get_rot_reward
        else:
            print("Controller type not found")
    ...
    def get_reward(self, state, action):
        return self.trans_or_rot_reward(self, state, action)

    def get_trans_reward(self, state, action):
        pos_state = state[0:3]
        vel_state = state[3:6]
...
class Agent():
    def __init__(self, alpha, beta, state_dim, action_dim, fc1_dim, fc2_dim, max_action,
                 ...
                 warmup, controller_type, save_folder):
             
        self.actor = ActorNetwork(alpha, state_dim, action_dim, fc1_dim, fc2_dim, max_action, name="actor_"+controller_type, model_ckpt_folder=save_folder)
        self.actor_target = ActorNetwork(alpha, state_dim, action_dim, fc1_dim, fc2_dim, max_action, name="target_actor_"+controller_type, model_ckpt_folder=save_folder)
        #self.actor_target.load_state_dict(self.actor.state_dict())
        
        self.critic_1 = CriticNetwork(beta, state_dim, action_dim, fc1_dim, fc2_dim, name="critic_1_"+controller_type, model_ckpt_folder=save_folder)
        self.critic_1_target = CriticNetwork(beta, state_dim, action_dim, fc1_dim, fc2_dim, name="target_critic_1_"+controller_type, model_ckpt_folder=save_folder)
        #self.critic_1_target.load_state_dict(self.critic_1.state_dict())
        
        self.critic_2 = CriticNetwork(beta, state_dim, action_dim, fc1_dim, fc2_dim, name="critic_2_"+controller_type, model_ckpt_folder=save_folder)
        self.critic_2_target = CriticNetwork(beta, state_dim, action_dim, fc1_dim, fc2_dim, name="target_critic_2_"+controller_type, model_ckpt_folder=save_folder)
        ...
        self.reward_computer = RewardComputer(self.approach_direction, self.reward_type, self.reward_parameters, 
                                              self.docking_ports, self.docking_settings, self.controller_type)
